import { EnhancedMarkdownTransformer } from '../../src/transform/enhancedMarkdownTransformer';
import type { Page } from '../../src/models/entities';
import type { ConfluenceApi } from '../../src/confluence/api';
import type { EnhancedTransformContext } from '../../src/transform/enhancedMarkdownTransformer';
...
describe('Download Queue Basic Functionality', () => {
  ...
  beforeEach(() => {
    mockApi = {
      ...
      getUser: jest.fn(),
    } as Partial<ConfluenceApi> as jest.Mocked<ConfluenceApi>;
  ...
  it('should return discovered page IDs from list-children macro', async () => {
    // Mock API to return child pages
    (mockApi.getChildPages as jest.Mock).mockResolvedValue({
      results: [
        { id: 'child1', title: 'Child Page 1', type: 'page' },
