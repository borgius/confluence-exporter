...
describe('ErrorClassifier', () => {
  ...
  describe('Content Error Classification', () => {
    it('should classify invalid markup as content error', () => {
      ...
      expect(classified.category).toBe('confluence');
      expect(classified.severity).toBe('medium');
      expect(classified.retryable).toBe(false);
  ...
  describe('Filesystem Error Classification', () => {
    it('should classify ENOENT as filesystem error', () => {
      ...
      const classified = classifier.classifyError(error, { operation: 'file_write', timestamp: Date.now() });
  ...
  describe('Configuration Error Classification', () => {
    it('should classify config errors as non-retryable', () => {
      ...
      const classified = classifier.classifyError(error, { operation: 'config_load', timestamp: Date.now() });
  ...
  describe('Validation Error Classification', () => {
    it('should classify validation errors correctly', () => {
      ...
      const classified = classifier.classifyError(error, { operation: 'validation', timestamp: Date.now() });
  ...
  describe('Error Statistics', () => {
    beforeEach(() => {
      ...
      classifier.classifyError(new Error('ENOENT'), { operation: 'file_write', timestamp: Date.now() });
      classifier.classifyError(new Error('Authentication failed'), { operation: 'api_call', timestamp: Date.now() });
    });
    ...
    it('should count retryable vs non-retryable errors', () => {
      const stats = classifier.getStats();
      expect(stats.retryableErrors).toBe(3); // network, rate_limit, unknown
  ...
  describe('Error Context Handling', () => {
    ...
    it('should handle missing context gracefully', () => {
      ...
      expect(classified.context.operation).toBe('unknown');
    });
  });
